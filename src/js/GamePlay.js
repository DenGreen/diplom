...
export default class GamePlay {
  ...
  radiusTurn(index, strokeLength = 2) {
    const rez = [];
    const row = Math.floor(index / 8) + 1;
    const col = Math.floor(index % 8) + 1;

    for (let i = 1; i <= strokeLength; i += 1) {
        let rowOne = row - i < 1 || row - i > 8 ? row : row - i;
        let rowTwo = row + i >  8 || row + i < 1 ? row : row + i;
        
        for(let y = 0; y <= strokeLength + 1; y += 1) {
          let colOne = col + y - 1 < 1 || col + y - 1 > 8 ? col : col + y - 1;

          rez.push(8 * (rowOne - 1) + (colOne - 1));
          rez.push(8 * (rowTwo - 1) + (colOne - 1));
          
        }

        for(let y = 1; y <= strokeLength; y += 1) {
          let colRight = col + y  > 8 ? col : col + y;
          let colLeft = col - y  < 1 ? col : col - y;

          rez.push(8 * (row - 1) + (colRight - 1));
          rez.push(8 * (row - 1) + (colLeft - 1));
        }
    }
